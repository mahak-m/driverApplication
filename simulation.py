...
class Simulation:
    ...
    def run(self, initial_events: List[Event]) -> Dict[str, float]:
        ...
        # remove events until the queue is empty
        while self._events.is_empty() is False:
            chosen_event = self._events.remove()

            # execute that event
            if isinstance(chosen_event, Event):
                # DO the event
                a = chosen_event.do(self._dispatcher, self._monitor)

                # add the events into the sublist and do them
                for i in a:
                    self._events.add(i)

        # get a report from the monitor and return
        the_report = self._monitor.report()
        return the_report
