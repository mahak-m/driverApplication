...
def test_simulation_run() -> None:
    ...
    assert len(events) == 12  # finally works now
    sim = Simulation()
    report = sim.run(events)
    assert len(report) == 3  # finally works now
...
def test_special_events() -> None:
    ...
    assert rider.status == 'satisfied'  # finally works now
    ...
    # Testing
    cancel = Cancellation(0, rider)  # finally works now
    result = cancel.do(dispatcher, monitor)  # finally works now
    assert result == []  # finally works now
    assert rider.status == 'cancelled'  # finally works now
